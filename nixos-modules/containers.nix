  ...
  config = lib.mkIf (lib.hasAttr "containers" cfgNamespace) {
    ...
    containers =
      let
        baseIP = "192.168.100";
        hostIP = "${baseIP}.10";
        containerNames = builtins.attrNames cfgNamespace.containers;
        getContainerIP =
          name:
          let
            index = lib.lists.findFirstIndex (n: n == name) null containerNames;
          in
          "${baseIP}.${toString (index + 11)}";
      in
      lib.mapAttrs (name: container: {
        ...
        privateNetwork = true;
        hostAddress = hostIP;
        localAddress = getContainerIP name;
        privateUsers = "pick";
        # Mount secret key
        bindMounts.sops = {
